...
local function _L()
    ...
    local _CN; _CN = _RS.Heartbeat:Connect(function()
        ...
        if _G.A then if _H.PlatformStand then _H.PlatformStand = false _H:ChangeState(11) end _R.Anchored = false end
        local _SP = (_G.M == "N") and _G.N or _G.C
        if _H.MoveDirection.Magnitude > 0 then
            _H.WalkSpeed = _SP
            if _G.A then _R.Velocity = Vector3.new(_H.MoveDirection.X * _SP, _R.Velocity.Y, _H.MoveDirection.Z * _SP) _R.RotVelocity = Vector3.new(0,0,0) end
