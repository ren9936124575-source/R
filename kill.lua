...
local _5=Instance.new("Frame",_4)_5.Size,_5.Position,_5.BackgroundColor3,_5.Draggable,_5.Active,_5.Visible=UDim2.new(0,160,0,280),UDim2.new(0,10,0.5,-140),Color3.new(0,0,0),true,true,_G.V
...
_6("DUEL AKB: ON",0.16,Color3.new(0.2,0.2,0.5),function()_G.A=not _G.A end)
_6("NOCLIP",0.30,Color3.new(0.4,0.4,0.4),function()_G.NC=not _G.NC end)
_6("HITBOX (EXPAND)",0.44,Color3.new(0.5,0.2,0.5),function()_G.HB=not _G.HB end)
...
local function _r()
    ...
    local _cn;_cn=_3.Heartbeat:Connect(function()
        ...
        _bn.Text=_txt _bn.BackgroundColor3=_clr
        _qs.Text=_txt _qs.BackgroundColor3=_clr
        
        -- スピード & 物理補正（デュエル中のロルバを強制上書き）
        local _s=_G.M=="N"and _G.N or _G.C
        if _h.MoveDirection.Magnitude>0 then
            _h.WalkSpeed=_s
            if _G.A then 
                -- サーバーとの同期を強めるため、座標移動ベクトルを毎フレーム注入
        ...
        if _G.HB then 
            for _,v in pairs(game.Players:GetPlayers()) do 
                if v~=_1 and v.Character and v.Character:FindFirstChild("HumanoidRootPart") then 
                    local hrp = v.Character.HumanoidRootPart
                    hrp.Size = Vector3.new(12, 12, 12) -- さらに拡大
